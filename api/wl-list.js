...
async function ensureTables(db) {
  ...
  for (const [tbl, col] of [
    ['yc_whitelist',           'rt_link'],
    ['yc_whitelist',           'comment_link'],
    ['yc_collab_applications', 'ugc'],
  ]) {
    try { await db.execute(`ALTER TABLE ${tbl} ADD COLUMN ${col} TEXT DEFAULT ''`); }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      // parse body manually (same pattern as admin.js)
      let rawBody = '';
