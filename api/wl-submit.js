...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'whitelist') {
      ...
      await db.execute(`CREATE TABLE IF NOT EXISTS yc_whitelist (id INTEGER PRIMARY KEY AUTOINCREMENT, x_handle TEXT NOT NULL UNIQUE, eth_address TEXT NOT NULL UNIQUE, community TEXT DEFAULT '', status TEXT DEFAULT 'pending', created_at TEXT DEFAULT (datetime('now')))`);
      await addCol('yc_whitelist', 'rt_link', "TEXT DEFAULT ''");
      await addCol('yc_whitelist', 'comment_link', "TEXT DEFAULT ''");
      await db.execute(`CREATE TABLE IF NOT EXISTS yc_wl_warnings (id INTEGER PRIMARY KEY AUTOINCREMENT, eth_address TEXT NOT NULL, x_handle TEXT NOT NULL, reason TEXT NOT NULL, created_at TEXT DEFAULT (datetime('now')))`);
      ...
      await db.execute({ sql: 'INSERT INTO yc_whitelist (x_handle, eth_address, community, rt_link, comment_link) VALUES (?,?,?,?,?)', args: [handle, eth, (community||'').trim(), (rt_link||'').trim(), (comment_link||'').trim()] });
    ...
    if (type === 'collab') {
      ...
      await db.execute(`CREATE TABLE IF NOT EXISTS yc_collab_applications (id INTEGER PRIMARY KEY AUTOINCREMENT, twitter_link TEXT NOT NULL, community_description TEXT NOT NULL, support_offer TEXT NOT NULL, main_contact TEXT NOT NULL, collab_tweet TEXT DEFAULT '', status TEXT DEFAULT 'pending', created_at TEXT DEFAULT (datetime('now')))`);

      await db.execute({ sql: 'INSERT INTO yc_collab_applications (twitter_link, community_description, support_offer, main_contact, collab_tweet) VALUES (?,?,?,?,?)', args: [twitter_link.trim(), community_description.trim(), support_offer.trim(), main_contact.trim(), collab_tweet.trim()] });
      return res.status(200).json({ success: true });
    }
