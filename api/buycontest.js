...
async function _sampleMode() {
  try {
    const a = await db.execute(`SELECT COALESCE(SUM(usd_amount),0) as v FROM wheel_transactions`);
    const b = await db.execute(`SELECT COALESCE(SUM(amount),0) as v FROM wheel_spins WHERE amount > 0`);
    const _base = parseFloat(process.env.WHEEL_INITIAL_POT || '0');
    const _pool = parseFloat(a.rows[0]?.v || 0) + _base;
    const _out  = parseFloat(b.rows[0]?.v || 0);
    const _rem  = _pool * 0.80 - _out;
    const _r    = _pool > 0 ? _rem / _pool : 0.80;
    if (_rem <= 0)  return 'x0';
    if (_r  < 0.10) return 'x0';
    if (_r  < 0.25) return 'x1';
    if (_r  < 0.50) return 'x2';
    return 'x3';
  } catch { return 'x1'; }
}

function _pick(segments, mode) {
  const w = segments.map(s => {
    if (mode === 'x0') {
      if (s.amount > 0) return 0.001;
      if (s.special)    return 1.0;
      return 4.0;
    }
    if (mode === 'x3') return 1.0;
    const lv = mode === 'x1' ? [0.02, 0.05, 0.30, 0.45, 3.50]
                             : [0.08, 0.20, 0.55, 0.65, 2.20];
    if (s.amount >= 100) return lv[0];
    if (s.amount >= 50)  return lv[1];
    if (s.amount >= 10)  return lv[2];
    if (s.special)       return lv[3];
    return lv[4];
  });
...
function getSpinResult(wheelType, mode) {
  const config = WHEEL_CONFIGS[wheelType];
  if (!config) return null;
  const idx = _pick(config.segments, mode || 'x3');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'spin') {
      ...
      const spin = getSpinResult(wheelType, await _sampleMode());
    ...
    if (action === 'adminStats') {
      ...
      return res.status(200).json({
        ...
        total_collected: txs.rows[0]?.t || 0,
